...
pub use self::signup_token::{SignupToken, SignupTokenRef};
use ink_lang as ink;
...
mod signup_token {
    ...
    mod tests {
        ...
        fn mint_works() {
            ...
            let mut signup_token = SignupToken::new();
            // Token 1 does not exists.
            assert_eq!(signup_token.owner_of(1), None);
            // Alice does not owns tokens.
            assert_eq!(signup_token.balance_of(accounts.alice), 0);
            // Create token Id 1.
            assert_eq!(signup_token.mint(1), Ok(()));
            // Alice owns 1 token.
            assert_eq!(signup_token.balance_of(accounts.alice), 1);
        }
        ...
        fn mint_existing_should_fail() {
            ...
            assert_eq!(signup_token.owner_of(1), Some(accounts.alice));
            // Cannot create  token Id if it exists.
            // Bob cannot own token Id 1.
            assert_eq!(signup_token.mint(1), Err(Error::TokenExists));
        }
        ...
        fn transfer_works() {
            ...
            assert_eq!(signup_token.balance_of(accounts.bob), 0);
            // The first Transfer event takes place
            assert_eq!(1, ink::env::test::recorded_events().count());
            // Alice transfers token 1 to Bob
            assert_eq!(signup_token.transfer(accounts.bob, 1), Ok(()));
            // The second Transfer event takes place
            assert_eq!(2, ink::env::test::recorded_events().count());
            // Bob owns token 1
            assert_eq!(signup_token.balance_of(accounts.bob), 1);
        }
        ...
        fn invalid_transfer_should_fail() {
            ...
            assert_eq!(
                signup_token.transfer(accounts.bob, 2),
                Err(Error::TokenNotFound)
            );
            // Token Id 2 does not exists.
            assert_eq!(signup_token.owner_of(2), None);
            // Create token Id 2.
            assert_eq!(signup_token.mint(2), Ok(()));
            // Alice owns 1 token.
            assert_eq!(signup_token.balance_of(accounts.alice), 1);
            // Token Id 2 is owned by Alice.
            assert_eq!(signup_token.owner_of(2), Some(accounts.alice));
            // Set Bob as caller
            set_caller(accounts.bob);
            // Bob cannot transfer not owned tokens.
            assert_eq!(
                signup_token.transfer(accounts.eve, 2),
        ...
        fn approved_transfer_works() {
            ...
            assert_eq!(signup_token.approve(accounts.bob, 1), Ok(()));
            // Set Bob as caller
            set_caller(accounts.bob);
            // Bob transfers token Id 1 from Alice to Eve.
            assert_eq!(
                signup_token.transfer_from(accounts.alice, accounts.eve, 1),
                Ok(())
            );
            // TokenId 3 is owned by Eve.
            assert_eq!(signup_token.owner_of(1), Some(accounts.eve));
            // Alice does not owns tokens.
            assert_eq!(signup_token.balance_of(accounts.alice), 0);
            // Bob does not owns tokens.
            assert_eq!(signup_token.balance_of(accounts.bob), 0);
            // Eve owns 1 token.
            assert_eq!(signup_token.balance_of(accounts.eve), 1);
        }
        ...
        fn approved_for_all_works() {
            ...
            assert_eq!(signup_token.mint(2), Ok(()));
            // Alice owns 2 tokens.
            assert_eq!(signup_token.balance_of(accounts.alice), 2);
            // Approve token Id 1 transfer for Bob on behalf of Alice.
            assert_eq!(
                signup_token.set_approval_for_all(accounts.bob, true),
                Ok(())
            );
            // Bob is an approved operator for Alice
            assert!(signup_token.is_approved_for_all(accounts.alice, accounts.bob));
            // Set Bob as caller
            set_caller(accounts.bob);
            // Bob transfers token Id 1 from Alice to Eve.
            assert_eq!(
            ...
            assert_eq!(
                signup_token.transfer_from(accounts.alice, accounts.eve, 2),
                Ok(())
            );
            // Bob does not own tokens.
            assert_eq!(signup_token.balance_of(accounts.bob), 0);
            // Eve owns 2 tokens.
            assert_eq!(signup_token.balance_of(accounts.eve), 2);
            // Remove operator approval for Bob on behalf of Alice.
            set_caller(accounts.alice);
            assert_eq!(
                signup_token.set_approval_for_all(accounts.bob, false),
                Ok(())
            );
        ...
        fn not_approved_transfer_should_fail() {
            ...
            assert_eq!(signup_token.balance_of(accounts.eve), 0);
            // Set Eve as caller
            set_caller(accounts.eve);
            // Eve is not an approved operator by Alice.
            assert_eq!(
                signup_token.transfer_from(accounts.alice, accounts.frank, 1),
                Err(Error::NotApproved)
            );
            // Alice owns 1 token.
            assert_eq!(signup_token.balance_of(accounts.alice), 1);
        ...
        fn burn_works() {
            ...
            assert_eq!(signup_token.owner_of(1), Some(accounts.alice));
            // Destroy token Id 1.
            assert_eq!(signup_token.burn(1), Ok(()));
            // Alice does not owns tokens.
            assert_eq!(signup_token.balance_of(accounts.alice), 0);
        ...
        fn burn_fails_token_not_found() {
            ...
            assert_eq!(signup_token.burn(1), Err(Error::TokenNotFound));
        }
        ...
        fn burn_fails_not_owner() {
            ...
            set_caller(accounts.eve);
            assert_eq!(signup_token.burn(1), Err(Error::NotOwner));
        }
